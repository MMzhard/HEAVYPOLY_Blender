...
# Function to disable keymap conflict
def disable_default_kmi(km=None, idname=None, retries=5):
    wm = bpy.context.window_manager

    if not (km and idname) or retries < 1:
        return

    # 全キー設定構成から走査して該当の再生キーを確実にオフにする
    for kc_name in ['Blender', 'Blender User']:
        if kc_name in wm.keyconfigs:
            kc = wm.keyconfigs[kc_name]
            if km in kc.keymaps:
...
def disable_specific_kmi(km=None, idname=None, type=None, value=None, shift=None, ctrl=None, alt=None, retries=5):
    wm = bpy.context.window_manager
    if not (km and idname) or retries < 1:
        return
...
def register():
    ...
    # 4.x以降で上位割り込みされる Screen / Frames の再生処理を無効化
    disable_default_kmi('Screen', 'screen.animation_play')
    ...
    for space_name, id_name, key_type, key_val in [
        ("3D View Tool: Move", "transform.translate", 'LEFTMOUSE', 'CLICK_DRAG'),
        ("Pose", "transform.translate", 'LEFTMOUSE', 'CLICK_DRAG'),
        ("Mesh", "wm.call_menu", 'X', 'PRESS'),
        ("Grease Pencil Edit Mode", "wm.call_menu", 'X', 'PRESS'),
    ]:
        kmi = get_active_kmi(space_name, idname=id_name, type=key_type, value=key_val, shift=False, ctrl=False, alt=False)
        if kmi:
            kmi.active = False

    kmi = get_active_kmi("Frames", idname="screen.animation_play", type='SPACE', shift=True, ctrl=True, alt=False)
    if kmi:
        kmi.active = False
